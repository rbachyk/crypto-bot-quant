...
**Implemented per-strategy execution model (`configs/strategies.yaml`, `src/strategies/candidates.py`; current `STRATEGY_VERSION strat_0009`).** Each candidate emits a `Signal` carrying its OWN entry + exit geometry, so families no longer share one engine geometry:
