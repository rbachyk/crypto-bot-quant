...
**Implemented per-strategy execution model (`configs/strategies.yaml`, `src/strategies/candidates.py`; current `STRATEGY_VERSION strat_0008`).** Each candidate emits a `Signal` carrying its OWN entry + exit geometry, so families no longer share one engine geometry:
...
- **Multiple-testing significance:** the deflated Sharpe must clear `min_deflated_sharpe`. It is a PSR-style probability in [0,1] computed from the POOLED per-trade OOS R-multiples across the folds — n_obs is the autocorrelation-discounted trade count and skew/kurtosis come from the same sample — deflated by the expected max of the selection trials genuinely compared (the side-decision variants are wired through as trials; a single-config run has one trial, so the deflation collapses to plain PSR). Sample size therefore matters: a 25-trade fluke can no longer score like a 500-trade edge. (0.5 = "more likely than not a real edge net of multiple testing"; a genuinely edgeless strategy averages below 0.5 — the anti-luck guard that stops directional folds passing by chance.)
...
- **Anti-overfit discipline observed in practice:** parameter values are chosen on the TRAIN folds, the hold-out is read once; changes that lift in-sample metrics but collapse the locked hold-out (e.g. basis funding-confirmation, an over-tight band cap) are rejected as overfits, not adopted. The side decision (long/short enable) is likewise fitted ONLY on the pre-hold-out window — `holdout_split`/`pre_holdout_inputs` in `walkforward.py` is the single shared boundary, so the locked hold-out evaluates a configuration it never influenced.
