...
- **Proposed next (need new feature columns first):** beta-residual cross-sectional mean-reversion (the correct fix for xsection — trade the idiosyncratic residual after regressing out BTC), cointegration/pairs spread reversion, and multi-day cross-sectional momentum. These require new features (beta residual, cointegration spread, multi-day return) added to the pipeline.
